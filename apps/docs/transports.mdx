...
| Direct WebRTC data channel | `webrtc.ts`    | The direct P2P fast path, opt-in via `WRAPPER_P2P`. |
...
behavior is unchanged. Turn it on per session on both ends:

```bash
# host
WRAPPER_P2P=1 wrapper shell-host          # then Ctrl+\ s to share
# viewer
WRAPPER_P2P=1 wrapper attach --relay -i <sessionId>
```
