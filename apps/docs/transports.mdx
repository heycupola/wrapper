...
- Data channels are DTLS-encrypted between peers. Viewer input prefers the
  direct channel, but the host keeps relay output available for fallback and
  mixed relay/P2P viewers. Relay infrastructure can therefore process fallback
  terminal output after TLS termination; Wrapper does not claim zero knowledge.
