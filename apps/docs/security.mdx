...
Run `bun run audit` from the repository root. One advisory is scoped out:

- Bun identifies Wrapper's private `ip@2.1.0` workspace shim by package name.
  The vulnerable registry implementation is absent; the tested shim uses
  maintained `ipaddr.js`.
